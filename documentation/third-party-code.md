...
| `better-sqlite3` | 13.0.2 | Provides a small synchronous SQLite driver suited to a local application without a separate database server. |
...
No ORM, CSS framework, component library, external API client or hosted database SDK is used. Node.js built-in modules such as `fs`, `path` and `os` are part of the runtime and are not third-party code.
